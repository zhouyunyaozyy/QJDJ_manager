    ...
    style="width: 100%">
      <el-table-column
        ...
        <el-table-column
          ...
          label="位置"
          min-width="120" align='center'>
          <template slot-scope='scope'>首页轮播图</template>
        </el-table-column>
        <el-table-column
          prop="remark"
          label="备注"
    ...
    methods: {
      getTableData () {
        this.$axios({
          ...
          data: {goods_activity_category_id: 1},
          fuc: (res) => {
      ...
      addSlideshow () {
        this.$router.push('/interConfig/indexManage/indexSlideshowDetail')
      },
      editSlideShow (row) {
        this.$router.push({path: '/interConfig/indexManage/indexSlideshowDetail', query: {goods_activity_carousel_id: row.goods_activity_carousel_id}})
      },
      removeSlideShow (row) {
        this.$axios({
          type: 'post',
