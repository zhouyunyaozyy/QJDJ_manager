<template>
  <div class="indexOperatingDetail">
    <el-form ref="form" :model="form" label-width="120px" :rules='rules'>
      ...
      <el-form-item label='选择模板' prop='template_id' v-if='form.type == 2'>
      ...
      <el-form-item label='分类数量' prop='category_num' v-if='form.type == 2 && (form.template_id == 1 || form.template_id == 3)'>
      ...
      <el-form-item v-if='form.type == 2 && form.template_id == 1'>
        <el-row>
      ...
      <el-form-item v-if='form.type == 2 && form.template_id == 3'>
        <el-row>
          <el-col :span='10'>
            <el-form-item :label='`第${index + 1}个名称`' v-for='(item, index) in form.category_info' :key='index' required>
              <el-input v-model="item.category_name" placeholder='输入分类名称' :maxlength='40'></el-input>
            </el-form-item>
          </el-col>
          <el-col :span='10'>
            <el-form-item label='类别' v-for='(item, index) in form.category_info' :key='index' required>
              <el-select v-model='item.category_type' placeholder="请选择">
                <el-option
                  label="商家"
                  :value="2">
                </el-option>
        ...
        this.$axios({
          ...
          fuc: (res) => {
            if (res.code == 200) {
              ...
              if (res.data.category_info && res.data.category_info.length > 0) {
      ...
      submit () {
        this.$refs['form'].validate((valid) => {
          if (valid) {
            ...
            this.$axios({
              type: 'post',
              url: '/goods/activityodedit',
              data: _form,
              fuc: (res) => {
                if (res.code == 200) {
                  this.$message.success('操作成功')
                  this.$deleteOneTag('/interConfig/indexManage/indexOperatingList')
                }
              }
            })
          }
